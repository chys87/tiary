...
/**
 * @file		common/md5.cpp
 * @author		L. Peter Deutsch <ghost@aladdin.com>; chys <admin@chys.info>
...
/******** Original copyright info by Deutsch ************************
...
md5_process(MD5Context *pms, const uint8_t *data /*[64]*/) noexcept
{
	...
	uint32_t X[16];
#else /* Define storage for little-endian CPUs. */
	uint32_t xbuf[16];
	const uint32_t *X;
#endif

#ifdef TIARY_BIG_ENDIAN
	/*
...
md5_finish(MD5Context *pms) noexcept
{
	...
	uint64_t msglen = pms->count;
	/* Pad to 56 bytes mod 64. */
	md5_append(pms, pad, ((55 - size_t(msglen) / 8) % 64) + 1);

	union {
		uint64_t data;
		uint8_t data_byte[8];
	};
#ifdef TIARY_BIG_ENDIAN // Big-endian CPU. Need to reverse
	data = bswap64 (msglen);
#else
	data = msglen;
#endif
	/* Append the length. */
	md5_append(pms, data_byte, 8);
